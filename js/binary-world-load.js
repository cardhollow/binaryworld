...
function detectAndDecode(buffer,text,url){
    ...
    if(bytes.length>=2&&bytes[0]===0x42&&bytes[1]===0x57){
        ...
        return{format:'BW',world:window.BinaryWorld.BW.createWorld(window.BinaryWorld.BW.readFile(bytes))};
    }
    var looksJSON=false;
    try{var trimmed=(text||'').trim();looksJSON=trimmed.charAt(0)==='{'||trimmed.charAt(0)==='['}catch(e){}
    if(looksJSON||/\.json(?:[?#].*)?$/i.test(url||'')){
        var data=JSON.parse(text);
        if(window.BinaryWorld&&typeof window.BinaryWorld.normalizeJSONWorld==='function')return{format:'JSON',world:window.BinaryWorld.normalizeJSONWorld(data)};
...
async function loadRemoteWorld(url){
    ...
    var buffer=await response.arrayBuffer(),text='';
    try{text=new TextDecoder().decode(new Uint8Array(buffer))}catch(e){}
    var decoded=detectAndDecode(buffer,text,url),saved=await saveTemporaryProject(decoded.world);
    ...
    await window.BinaryWorldStorage.loadByKey(saved.key);
}
